export enum ICONS_STROKES {
  PRIMARY_DEFAULT_COLOR = 'rgb(130, 219, 247)',
  PRIMARY_HOVER_COLOR = 'rgb(76, 143, 164)',
}

export enum BTN_BACKGROUND_VARIABLES {
  PRIMARY_GRADIENT_BUTTON = 'linear-gradient(45deg, rgb(76, 143, 164) 0%,rgb(130, 219, 247) 45.313%,rgb(192, 237, 251) 100%)',
  PRIMARY_GRADIENT_DISABLED_BUTTON = 'linear-gradient(145.32deg, rgba(215, 237, 237, 0.16) -29.161%, rgba(204, 235, 235, 0) 100%)',
}

export enum TEXT_COLORS_VARIABLES {
  PRIMARY_TEXT = 'rgb(255, 255, 255)',
}

export enum BACKGROUND_COLORS_VARIABLES {
  SIDEBAR_BACK = 'rgb(13, 15, 16)',
  GENERAL_BACK = 'rgb(19, 22, 25)',
  ITEMS_BACK = 'rgb(31, 34, 38)',
  ITEMS_GRADIENT_BACK = 'linear-gradient(117.24deg, rgb(86, 98, 104) -70.139%,rgba(13, 15, 16, 0) 126.787%)',
}

export enum SHARED_COLORS_VARIABLES {
  LINK_LIGHT_COLOR = 'rgb(182, 240, 156)',
  LINK_BRIGHT_COLOR = 'rgb(17, 175, 44)',
  ERROR_COLOR = 'rgb(208, 48, 47)',
}

export enum BADGE_BACKGROUND_VARIABLES {
  ACTIVE_BADGE = 'linear-gradient(45.00deg, rgb(3, 255, 160) 0%,rgb(35, 161, 155) 100%)',
  NEUTRAL_BADGE = ' linear-gradient(145.32deg, rgba(215, 237, 237, 0.16) -41.478%,rgba(204, 235, 235, 0) 100%)',
}
